 ...
 * 
 * It currently relies on 6 to N tables with the format. Every Index created by
 * the Indexable interface gets its own table.
 * 
 * VertexTable
 * <table border=1>
 * <thead>
...
public class AccumuloGraph implements Graph, KeyIndexableGraph, IndexableGraph {
	...
	private void setupWriters() {
		try {
			writer = config.getConnector().createMultiTableBatchWriter(
	 ...
	 */
	public static AccumuloGraph open(Configuration properties)
	...
	private BatchScanner getBatchScanner(String table) {
		try {
			BatchScanner x = config.getConnector().createBatchScanner(table,
					config.getAuthorizations(), config.getQueryThreads());
		...
		try {
			if (!config.skipExistenceChecks()) {
				...
				while (iter.hasNext()) {
					...
					Object val = AccumuloByteSerializer.desserialize(entry
							.getValue().get());
	...
	public void removeVertex(Vertex vertex) {
		...
		try {
			// Set up Deleters
			edgedeleter = config.getConnector().createBatchDeleter(
					config.getEdgeTable(), config.getAuthorizations(),
					config.getQueryThreads(), config.getBatchWriterConfig());
			vertexdeleter = config.getConnector().createBatchDeleter(
					config.getVertexTable(), config.getAuthorizations(),
					config.getQueryThreads(), config.getBatchWriterConfig());
			...
			while (iter.hasNext()) {
				...
				if (k.getColumnFamily().equals(TOUTEDGE)
						|| k.getColumnFamily().equals(TINEDGE)) {
					ranges.add(new Range(k.getColumnQualifier().toString()
							.split(IDDELIM)[1]));

					Mutation vm = new Mutation(k.getColumnQualifier()
							.toString().split(IDDELIM)[0]);
					vm.putDelete(
							invert(k.getColumnFamily()),
	...
	private void clearIndex(Type type, Object id) {
		...
		while (iter.hasNext()) {
			...
			try {
				del = config.getConnector().createBatchDeleter(table,
						config.getAuthorizations(),
				...
				IteratorSetting is = new IteratorSetting(10, "getEdgeFilter",
						RegExFilter.class);
				RegExFilter.setRegexs(is, null, null, regex.toString(), null,
						false);
	...
	public Iterable<Vertex> getVertices() {
		...
		return new ScannerIterable<Vertex>(this, scan) {
			...
			public Vertex next(Iterator<Entry<Key, Value>> iterator) {
				...
				return new AccumuloVertex(AccumuloGraph.this, e.getKey()
						.getRow().toString());
			}
		};
	}
		...
		if (getIndexedKeys(Vertex.class).contains(key)) {
			...
			return new ScannerIterable<Vertex>(this, s) {
				...
				public Vertex next(Iterator<Entry<Key, Value>> iterator) {
					...
					// TODO could also check local cache before creating a new
					// instance?
					return new AccumuloVertex(AccumuloGraph.this, iterator
			...
			if (val[0] != AccumuloByteSerializer.SERIALIZABLE) {
				...
				IteratorSetting is = new IteratorSetting(10, "filter",
						RegExFilter.class);
				RegExFilter.setRegexs(is, null, null, null,
						Pattern.quote(new String(val)), false);
	...
	public Edge addEdge(Object id, Vertex outVertex, Vertex inVertex,
		...
		try {
			...
			m.put(INEDGE,
					(outVertex.getId().toString() + IDDELIM + myID).getBytes(),
					(IDDELIM + label).getBytes());
			vertexBW.addMutation(m);
			m = new Mutation(outVertex.getId().toString());
			m.put(OUTEDGE,
					(inVertex.getId().toString() + IDDELIM + myID).getBytes(),
					(IDDELIM + label).getBytes());
			vertexBW.addMutation(m);
		} catch (MutationsRejectedException e) {
			e.printStackTrace();
		...
		AccumuloEdge edge = new AccumuloEdge(this, myID, label, inVertex,
				outVertex);
		if (edgeCache != null) {
	...
	public void removeEdge(Edge edge) {
		...
		try {
			...
			m.putDelete(INEDGE, (outVert.toString() + IDDELIM + edge.getId()
					.toString()).getBytes());
			vertexBW.addMutation(m);
			m = new Mutation(outVert);
			m.putDelete(OUTEDGE, (inVert.toString() + IDDELIM + edge.getId()
					.toString()).getBytes());
			vertexBW.addMutation(m);
			...
			edgedeleter = config.getConnector().createBatchDeleter(
					config.getVertexTable(), config.getAuthorizations(),
					config.getQueryThreads(), config.getBatchWriterConfig());
			edgedeleter.setRanges(Collections.singleton(new Range(edge.getId()
					.toString())));
			edgedeleter.delete();
		} catch (MutationsRejectedException e) {
			e.printStackTrace();
	...
	public Iterable<Edge> getEdges() {
		...
		return new ScannerIterable<Edge>(this, scan) {
			...
			public Edge next(Iterator<Entry<Key, Value>> iterator) {
				...
				return new AccumuloEdge(AccumuloGraph.this, entry.getKey()
						.getRow().toString(), AccumuloByteSerializer
	...
	<T> Pair<Integer, T> getProperty(Type type, String id, String key) {
		...
		Iterator<Entry<Key, Value>> iter = s.iterator();
		if (iter.hasNext()) {
			toRet = AccumuloByteSerializer.desserialize(iter.next().getValue()
					.get());
		}
		s.close();
	...
	/**
	 * Sets the property. Requires a round-trip to Accumulo to see if the
	 * property exists iff the provided key has an index. Therefore, for best
	 * performance if at all possible create indices after bulk ingest.
	 * 
	 * @param type
	 * @param id
	 * @param key
	...
	<T> T removeProperty(Type type, String id, String key) {
		if (StringFactory.LABEL.equals(key) || SLABEL.equals(key)) {
			throw new RuntimeException("Cannot remove the "
					+ StringFactory.LABEL + " property.");
		}
	...
	Iterable<Edge> getEdges(String vertexId, Direction direction,
			String... labels) {
		Scanner scan = getVertexScanner();
	...
	Iterable<Vertex> getVertices(String vertexId, Direction direction,
			String... labels) {
		Scanner scan = getVertexScanner();
		scan.setRange(new Range(vertexId));
		if (direction.equals(Direction.IN)) {
	...
	public Type getType(String simple) {
		if (simple.equals(AccumuloEdge.class.getSimpleName())
				|| simple.equals(Edge.class.getSimpleName())) {
			return Type.Edge;
		} else if (simple.equals(AccumuloVertex.class.getSimpleName())
				|| simple.equals(Vertex.class.getSimpleName())) {
			return Type.Vertex;
		}
		return null;
	}
	...
	public <T extends Element> Index<T> createIndex(String indexName,
		...
		try {
			...
			return new AccumuloIndex<T>(getType(indexClass.getSimpleName()),
					this, indexName);
		} finally {
			s.close();
		}
	...
	public <T extends Element> Index<T> getIndex(String indexName,
		...
		try {
			...
			while (iter.hasNext()) {
				...
				if (k.getColumnFamily().toString()
						.equals(indexClass.getSimpleName())) {
					return new AccumuloIndex<T>(
							getType(indexClass.getSimpleName()), this,
							indexName);
				} else {
	...
	public Iterable<Index<? extends Element>> getIndices() {
		...
		try {
			...
			while (iter.hasNext()) {
				Key k = iter.next().getKey();
				toRet.add(new AccumuloIndex(getType(k.getColumnFamily()
						.toString()), this, k.getRow().toString()));
			}
			return toRet;
		} finally {
			scan.close();
	...
	public void dropIndex(String indexName) {
		...
		try {
			...
			deleter = config.getConnector().createBatchDeleter(
					config.getMetadataTable(), config.getAuthorizations(),
					config.getQueryThreads(), config.getBatchWriterConfig());
			deleter.setRanges(Collections.singleton(new Range(indexName)));
			deleter.delete();
			config.getConnector().tableOperations()
					.delete(config.getName() + "_index_" + indexName);
		} catch (TableNotFoundException e) {
			e.printStackTrace();
		} catch (AccumuloException e) {
			e.printStackTrace();
	...
	public <T extends Element> void dropKeyIndex(String key,
		...
		try {
			bd = config.getConnector().createBatchDeleter(table,
					config.getAuthorizations(), config.getMaxWriteThreads(),
					config.getBatchWriterConfig());
			w.addMutation(m);
	...
	public <T extends Element> void createKeyIndex(String key,
		...
		checkedFlush();
		// Re Index Graph
		BatchScanner scan = getReIndexBatchScanner(getType(elementClass
				.getSimpleName()));
		try {
			...
			BatchWriter bw = getIndexBatchWriter(getType(elementClass
					.getSimpleName()));
			while (iter.hasNext()) {
				Entry<Key, Value> entry = iter.next();
				Key k = entry.getKey();
				...
				mu.put(k.getColumnFamily().getBytes(), k.getRow().getBytes(),
						EMPTY);
				try {
					bw.addMutation(mu);
				} catch (MutationsRejectedException e) {
