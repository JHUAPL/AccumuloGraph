...
public final class AccumuloByteSerializer {
	...
	public static <T> T desserialize(byte[] target) {
		...
		switch (target[0]) {
		case BYTE:
			return (T) (Byte) Byte.parseByte(new String(target, 1,
					target.length - 1));
		case SHORT:
			return (T) (Short) Short.parseShort(new String(target, 1,
					target.length - 1));
		case CHARACTER:
			return (T) (Character) new String(target, 1, target.length - 1)
					.charAt(0);
		case INTEGER:
			return (T) (Integer) Integer.parseInt(new String(target, 1,
					target.length - 1));
		case LONG:
			return (T) (Long) Long.parseLong(new String(target, 1,
					target.length - 1));
		case FLOAT:
			return (T) (Float) Float.parseFloat(new String(target, 1,
					target.length - 1));
		case DOUBLE:
			return (T) (Double) Double.parseDouble(new String(target, 1,
					target.length - 1));
		case BOOLEAN:
		...
		case DATE:
			long millis = Long.parseLong(new String(target, 1,
					target.length - 1));
		...
		case ENUM:
			try {
				String[] s = new String(target, 1, target.length - 1)
						.split(":");
				Class<? extends Enum> clz = (Class<? extends Enum>) Class
						.forName(s[0]);
				return (T) Enum.valueOf(clz, s[1]);
			} catch (ClassNotFoundException cnfe) {
				throw new RuntimeException(
						"Unexpected error deserializing object.", cnfe);
			}
		case SERIALIZABLE:
			try {
				ByteArrayInputStream bin = new ByteArrayInputStream(target, 1,
						target.length);
				ObjectInputStream ois = new ObjectInputStream(bin);
				return (T) ois.readObject();
			} catch (IOException io) {
				throw new RuntimeException(
						"Unexpected error deserializing object.", io);
			} catch (ClassNotFoundException cnfe) {
				throw new RuntimeException(
						"Unexpected error deserializing object.", cnfe);
			}
	...
	public static byte[] serialize(Object o) {
		...
		try {
			...
			throw new RuntimeException(
					"Unexpected error writing to byte array.", io);
		} finally {
			bout.reset();
		}
	}
